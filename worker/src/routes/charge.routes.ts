...
charges.get('/website/:websiteId', async (c) => {
  const result = await c.env.NEXUS_OPS.prepare('SELECT * FROM additional_charges WHERE website_id = ? ORDER BY date DESC').bind(c.req.param('websiteId')).all();
  return c.json(camelCaseKeys(result.results));
});
...
charges.put('/:id', async (c) => {
  const { description, amount, date, billingType, status, notes } = await c.req.json();
  await c.env.NEXUS_OPS.prepare(`UPDATE additional_charges SET description=COALESCE(?,description), amount=COALESCE(?,amount),
...
charges.delete('/:id', async (c) => {
  await c.env.NEXUS_OPS.prepare('DELETE FROM additional_charges WHERE id = ?').bind(c.req.param('id')).run();

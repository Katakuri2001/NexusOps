...
websites.get('/:id', async (c) => {
  ...
  if (auth.role === 'TECHNICIAN') {
    const hasAccess = await c.env.NEXUS_OPS.prepare('SELECT 1 FROM technician_website_assignments WHERE technician_id IN (SELECT id FROM technicians WHERE user_id = ?) AND website_id = ?').bind(auth.userId, c.req.param('id')).first();
    if (!hasAccess) return c.json({ error: 'Access denied' }, 403);
  }

  const [hosting, database, server, plan] = await Promise.all([
    c.env.NEXUS_OPS.prepare('SELECT * FROM hosting_services WHERE website_id = ?').bind(c.req.param('id')).first(),
    c.env.NEXUS_OPS.prepare('SELECT * FROM database_services WHERE website_id = ?').bind(c.req.param('id')).first(),
    c.env.NEXUS_OPS.prepare('SELECT * FROM server_services WHERE website_id = ?').bind(c.req.param('id')).first(),
    c.env.NEXUS_OPS.prepare('SELECT * FROM plans WHERE website_id = ?').bind(c.req.param('id')).first(),
  ]);
...
websites.delete('/:id', authorize('OWNER'), async (c) => {
  await c.env.NEXUS_OPS.prepare('DELETE FROM websites WHERE id = ?').bind(c.req.param('id')).run();
...
websites.put('/:id/hosting', authorize('OWNER'), async (c) => {
  ...
  const existing = await c.env.NEXUS_OPS.prepare('SELECT id FROM hosting_services WHERE website_id = ?').bind(c.req.param('id')).first();
  if (existing) {
    await c.env.NEXUS_OPS.prepare(`UPDATE hosting_services SET provider=COALESCE(?,provider), status=COALESCE(?,status),
    ...
    await c.env.NEXUS_OPS.prepare(`INSERT INTO hosting_services (id,website_id,provider,status,cost,billing_cycle,start_date,due_date,notes)
...
websites.put('/:id/database', authorize('OWNER'), async (c) => {
  ...
  const existing = await c.env.NEXUS_OPS.prepare('SELECT id FROM database_services WHERE website_id = ?').bind(c.req.param('id')).first();
  if (existing) {
    await c.env.NEXUS_OPS.prepare(`UPDATE database_services SET provider=COALESCE(?,provider), status=COALESCE(?,status),
    ...
    await c.env.NEXUS_OPS.prepare(`INSERT INTO database_services (id,website_id,provider,status,monthly_cost,billing_cycle,start_date,due_date,notes)
...
websites.put('/:id/server', authorize('OWNER'), async (c) => {
  ...
  const existing = await c.env.NEXUS_OPS.prepare('SELECT id FROM server_services WHERE website_id = ?').bind(c.req.param('id')).first();
  if (existing) {
    await c.env.NEXUS_OPS.prepare(`UPDATE server_services SET provider=COALESCE(?,provider), status=COALESCE(?,status),
    ...
    await c.env.NEXUS_OPS.prepare(`INSERT INTO server_services (id,website_id,provider,status,plan,cost,billing_cycle,start_date,due_date,notes)
...
websites.put('/:id/plan', authorize('OWNER'), async (c) => {
  ...
  const existing = await c.env.NEXUS_OPS.prepare('SELECT id FROM plans WHERE website_id = ?').bind(c.req.param('id')).first();
  if (existing) {
    await c.env.NEXUS_OPS.prepare(`UPDATE plans SET name=COALESCE(?,name), description=COALESCE(?,description),
    ...
    await c.env.NEXUS_OPS.prepare(`INSERT INTO plans (id,website_id,name,description,price,billing_cycle,features,start_date,renewal_date)
...
websites.get('/:id/financial', async (c) => {
  const plan = await c.env.NEXUS_OPS.prepare('SELECT * FROM plans WHERE website_id = ?').bind(c.req.param('id')).first();
  const hosting = await c.env.NEXUS_OPS.prepare('SELECT * FROM hosting_services WHERE website_id = ?').bind(c.req.param('id')).first();
  const db = await c.env.NEXUS_OPS.prepare('SELECT * FROM database_services WHERE website_id = ?').bind(c.req.param('id')).first();
  const server = await c.env.NEXUS_OPS.prepare('SELECT * FROM server_services WHERE website_id = ?').bind(c.req.param('id')).first();
  const charges = await c.env.NEXUS_OPS.prepare('SELECT * FROM additional_charges WHERE website_id = ?').bind(c.req.param('id')).all();
...
websites.get('/:id/maintenance', async (c) => {
  const result = await c.env.NEXUS_OPS.prepare('SELECT * FROM maintenance_records WHERE website_id = ? ORDER BY created_at DESC').bind(c.req.param('id')).all();
...
websites.post('/:id/maintenance', async (c) => {
  ...
  const id = crypto.randomUUID();
  await c.env.NEXUS_OPS.prepare(`INSERT INTO maintenance_records (id,website_id,title,description,items,status,notes,is_internal,created_by_id)
...
websites.post('/:id/charges', authorize('OWNER'), async (c) => {
  ...
  const id = crypto.randomUUID();
  await c.env.NEXUS_OPS.prepare(`INSERT INTO additional_charges (id,website_id,description,amount,date,billing_type,status,notes)
...
websites.get('/:id/charges', async (c) => {
  const result = await c.env.NEXUS_OPS.prepare('SELECT * FROM additional_charges WHERE website_id = ? ORDER BY date DESC').bind(c.req.param('id')).all();
  return c.json(camelCaseKeys(result.results));
});

// GET /:id/timeline
websites.get('/:id/timeline', async (c) => {
  const result = await c.env.NEXUS_OPS.prepare('SELECT * FROM website_timeline WHERE website_id = ? ORDER BY created_at DESC').bind(c.req.param('id')).all();
  return c.json(camelCaseKeys(result.results));
});
...
websites.post('/:id/timeline', authorize('OWNER'), async (c) => {
  ...
  const id = crypto.randomUUID();
  await c.env.NEXUS_OPS.prepare('INSERT INTO website_timeline (id,website_id,title,description,icon) VALUES (?,?,?,?,?)').bind(id, c.req.param('id'), title, description||null, icon||null).run();
  return c.json({ id }, 201);
});
...
websites.get('/:id/notifications', async (c) => {
  const result = await c.env.NEXUS_OPS.prepare('SELECT * FROM notifications WHERE website_id = ? ORDER BY created_at DESC').bind(c.req.param('id')).all();
  return c.json(camelCaseKeys(result.results));
});
...
websites.patch('/:id/status', async (c) => {
  ...
  await c.env.NEXUS_OPS.batch([
    c.env.NEXUS_OPS.prepare('UPDATE websites SET status = ?, updated_at = datetime("now") WHERE id = ?').bind(status, c.req.param('id')),
    c.env.NEXUS_OPS.prepare('INSERT INTO website_status_history (id,website_id,status,reason,description,updated_by) VALUES (?,?,?,?,?,?)').bind(crypto.randomUUID(), c.req.param('id'), status, reason||null, description||null, auth.userId),
  ]);

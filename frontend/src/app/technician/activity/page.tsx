...
export default function TechnicianActivity() {
  ...
  useEffect(() => {
    api.getActivityLogs().then(l => setLogs(l.filter((x: any) => x.user?.name))).catch(() => {}).finally(() => setLoading(false));
  }, []);

  return (
    <AppShell>
      <div className="space-y-5 animate-fade-in">
        <PageHeader title="Activity" description="Recent system activity" />
              ...
              {logs.slice(0, 30).map((log: any) => (
                <div key={log.id} className="flex items-start gap-3 px-5 py-3">
                  <div className="w-8 h-8 rounded-lg bg-overlay flex items-center justify-center flex-shrink-0 mt-0.5">
                    <span className="text-xs font-bold text-muted">{log.action.charAt(0)}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm">
                      <span className="font-medium">{log.user?.name}</span>
                      <span className="text-muted"> {log.action.toLowerCase().replace(/_/g, ' ')} {log.entity}</span>
                    </p>
                  </div>

...
import { ArrowLeft, Globe, Server, Database, HardDrive, Wrench, Clock } from 'lucide-react';
import { formatDate, getDueDateLabel, getUrgencyColor, formatRelativeTime } from '@/lib/utils';

type Tab = 'overview' | 'services' | 'maintenance' | 'timeline';

export default function TechnicianWebsiteDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const w = await api.getWebsite(params.id as string);
        setWebsite(w);
  ...
  if (!website) return <AppShell><div className="text-center py-32 text-sm text-muted">Website not found or access denied</div></AppShell>;
          ...
          {tabs.map((t) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${
                tab === t.id ? 'bg-primary-subtle text-primary' : 'text-muted hover:text-foreground hover:bg-overlay'
              }`}
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <CardContent>
                <div className="space-y-3">
                  {[
                    ...
                    ...services.map(s => ({ label: s.name, status: s.data.status })),
                  ].map((item) => (
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {[
                    { label: 'Customer', value: website.customer?.user?.name || '—' },
                    { label: 'Type', value: website.websiteType || '—' },
                    { label: 'Launch', value: website.launchDate ? formatDate(website.launchDate) : '—' },
